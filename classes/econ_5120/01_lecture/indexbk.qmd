...
fetch <- c("DGS1", "DGS10","DGS3","DGS30")
if(params$refresh_fred  || !file.exists("data/int_rates.RDS")){
  dat <- map_df(fetch, fredr, frequency="m", observation_start=ymd(19600101))
  write_rds(dat, "data/int_rates.RDS")
}else{
  dat <- read_rds("data/int_rates.RDS")
}
...
::: footer
Fred Data: [Market Yield on U.S. 10 Year](https://fred.stlouisfed.org/series/DGS10)
:::

## U.S. Treasury Yields MacroTools
...
fred_params <- c(frequency="m", 
                   observation_start=ymd(19600101))

fetch <- c("DGS1", "DGS10","DGS3","DGS30")
dat <- getFredData(fetch_symbols = fetch, 
                   data_file_name = "int_rates2", 
                   data_dir = "data", 
...
## U.S. Treasury Yields YoY Change
...
if(params$refresh_fred  || !file.exists(data_file_path)){
  dat <- map_df(fetch, fredr, frequency="m", observation_start=ymd(19600101), units="ch1")
...
## U.S. 30 Year Treasury vs Mortgage
...
data_file_path <- file.path("data", paste0(data_file_name,".RDS"))
if(params$refresh_fred  || !file.exists(data_file_path)){
  dat <- map_df(fetch$series_id, fredr, frequency="m", observation_start=ymd(19750101)) %>% 
    left_join(fetch) 
  write_rds(dat, data_file_path)
}else{
  dat <- read_rds(data_file_path)
...
#| fig-width: 10
#| fig-height: 4.5
data_file_name <- "inter_risk_premium"

fetch <- tribble(
  ~series_id, ~series_name,
  "DGS10", "10 Year Treasury",
  "BAMLC0A1CAAAEY", "ICE BofA AAA",
  "BAMLC0A4CBBBEY", "ICE BofA BBB",
  "BAMLH0A0HYM2EY", "ICE BofA High Yield"
)

data_file_path <- file.path("data", paste0(data_file_name,".RDS"))
if(params$refresh_fred  || !file.exists(data_file_path)){
  dat <- map_df(fetch$series_id, fredr, frequency="m", observation_start=ymd(19970101)) %>% 
    left_join(fetch) 
  write_rds(dat, data_file_path)
}else{
  dat <- read_rds(data_file_path)
...
#| fig-width: 10
#| fig-height: 4.5
data_file_name <- "fed_balance_sheet"
...
data_file_path <- file.path("data", paste0(data_file_name,".RDS"))
if(params$refresh_fred  || !file.exists(data_file_path)){
  dat <- map_df(fetch$series_id, fredr, frequency="m", observation_start=ymd(20040101)) %>% 
    left_join(fetch) 
  write_rds(dat, data_file_path)
}else{
  dat <- read_rds(data_file_path)
...
dat %>%  
  ggplot(mapping = aes(x=date, y=value, fill=series_name)) + 
...
::: footer
Fred Data: [Fed Treasury Securities](https://fred.stlouisfed.org/series/TREAST) \| [Fed Mortgage Backed Securities](https://fred.stlouisfed.org/series/WSHOMCB)
:::

## U.S. 30 Year Treasury vs Mortgage Scatter
...
#| fig-height: 4.5
data_file_name <- "treas_v_mort"
fetch <- c("DGS30","MORTGAGE30US")
# add rename


data_file_path <- file.path("data", paste0(data_file_name,".RDS"))
if(params$refresh_fred  || !file.exists(data_file_path)){
  dat <- map_df(fetch, fredr, frequency="m", observation_start=ymd(19750101))
  write_rds(dat, data_file_path)
}else{
  dat <- read_rds(data_file_path)
}
...
dat %>% 
  ...
  select(-realtime_start, -realtime_end) %>% 
  ggplot(mapping = aes(x=DGS30, y=MORTGAGE30US, color=date)) + 
  geom_point() + 
